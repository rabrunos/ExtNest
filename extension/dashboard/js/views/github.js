...
export async function connectGitHub(refreshAll) {
  ...
  document.getElementById("deviceHint").textContent = `Expira em aproximadamente ${Math.ceil(begin.expires_in / 60)} minutos.`;
  ...
  while (!pollCancelled && Date.now() < expiresAt) {
    ...
    if (!response?.ok) throw new Error(response?.error || "Falha na autenticação GitHub.");
  }

  if (!pollCancelled) throw new Error("A autorização do GitHub expirou.");
}
...
export function renderRepoList() {
  ...
  for (const repo of state.repos.filter(x => !query || x.full_name.toLowerCase().includes(query))) {
